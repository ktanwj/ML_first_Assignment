...
for n_state in N_STATE_LIST:
    ...
    reward_results = []
    runtime_results = []
    frozen_lake = gym.make('FrozenLake-v1', desc=generate_random_map(size=n_state, seed=SEED), render_mode=None)
    ...
    for gamma in GAMMA_LIST:
        ...
        # plot heat map
        if n_state < 10:
            values_heat_map_with_env(frozen_lake, V, f"FL\Policy Iteration State Value for Gamma:{gamma}, State:{n_state}", size=(n_state,n_state), FILE_PATH=FOLDER_PATH + f'heatmap/pi_heatmap_g{gamma}_s{n_state}.png')
