...
for n_state in tqdm(N_STATE_LIST):
    ...
    for gamma in GAMMA_LIST:
        start = time.time()
        Q, V, pi, Q_track, pi_track = RL(frozen_lake).q_learning(gamma = gamma, n_episodes=N_ITERS)
        end = time.time()
        ...
        # plot heat map
        if n_state < 10:
            values_heat_map_with_env(frozen_lake, V, f"FL\Q-Learning State Value for Gamma:{gamma}, State:{n_state}", size=(n_state,n_state), FILE_PATH=FOLDER_PATH + f'heatmap/v_heatmap_g{gamma}_s{n_state}.png')

            fl_actions = {0: "←", 1: "↓", 2: "→", 3: "↑"}
            fl_map_size=(n_state,n_state)
            title=f"FL Q-Learning Mapped Policy for Gamma:{gamma}, State: {n_state}\nArrows represent best action"
