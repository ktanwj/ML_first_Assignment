...
class mlroseNeuralNetwork(mlrose.NeuralNetwork):
    def __init__(
        self,
        hidden_nodes=None,
        activation="relu",
...
def model_tuning(X_train, y_train, X_test, y_test, scores, data_name, verbose = False):
    ...
    tuned_parameters = [[{'algorithm':['random_hill_climb'], 'learning_rate': [0.001, 0.005 ,0.01, 0.05, 0.1],'max_attempts': [50, 100], 'restarts':[0, 3]}],
                        [{'algorithm':['simulated_annealing'], 'learning_rate': [0.001, 0.005 ,0.01, 0.05, 0.1],'max_attempts': [50, 100], 'restarts':[0, 3]}],
                        [{'algorithm':['genetic_alg'], 'learning_rate': [0.001, 0.005 ,0.01, 0.05, 0.1],'max_attempts': [50, 100], 'restarts':[0, 3]}]]
    algorithms = [mlrose.NeuralNetwork(hidden_nodes=[256], activation='relu',
                                        max_iters=300, bias=True, is_classifier=True, 
                  ...
                  mlrose.NeuralNetwork(hidden_nodes=[256], activation='relu',
                                        max_iters=300, bias=True, is_classifier=True, 
                                        early_stopping=False, clip_max=5, curve=True)]
    algorithm_names = ["Random Hill Climb","Simulated Annealing","Genetic Algorithm"]
    custom_scorer_function = make_scorer(custom_scorer, greater_is_better=False)
    ...
    for i in range(0, len(algorithms)):
        ...
        for score in scores:
            ...
            clf = GridSearchCV(algorithms[i], tuned_parameters[i], n_jobs=-1, cv=5,
                            scoring='%s' % score)
            ...
            test_df = pd.DataFrame(np.column_stack([algorithm_names[i], score, clf.best_params_, f1_score(y_test, y_pred), clf.refit_time_]), columns=validation_report_column_names)
            test_report = pd.concat([test_report, test_df])
            
            # plot learning curve
            print(f'best estimator: {clf.best_estimator_}')
            print(f'algorithm name: {algorithm_names[i]}')
            print(f'score: {score}')
            model = mlroseNeuralNetwork(
                    hidden_nodes=[256],
                    activation="relu",
                    algorithm=tuned_parameters[i][0]['algorithm'],
                    max_iters=300,
                    bias=True,
                    is_classifier=True,
            ...
            plot_learning_curve(model_name=algorithm_names[i], best_model=model, X = X_train, y=y_train, data_name=data_name, score='f1')
            ...
            file_path = f"classification_report/{algorithm_names[i]}_classification_report.txt"
        ...
        # save train_report
        train_validation_report.to_csv(f'model_perf/{algorithm_names[i]}_train_validation_report.csv')

        # save test_report
        test_report.to_csv(f'model_perf/{algorithm_names[i]}_test_report.csv')
...
def plot_validation_curve(data_name, title, x_lab, y_lab, model_name, model, score_metric, param_name, param_range):
    ...
    plt.legend(loc="best")
    plt.grid(True)
    plt.savefig(f'validation_curve/{model_name}_{param_name}_{score_metric}.png')
    plt.close()
...
def plot_learning_curve(model_name, best_model, X, y, score, data_name, cv=5):
    ...
    plt.legend(loc="best")
    plt.grid(True)
    plt.savefig(f'learning_curve/{model_name}_learning_curve.png')
    plt.show()

def plot_roc(model_name, best_model, X_test, y_test, data_name):
    # Predict probabilities for the test data
    y_pred = best_model.predict(X_test)
    y_prob = best_model.predicted_probs
    ...
    plt.grid(True)
    plt.savefig(f'ROC_curve/{model_name}_ROC.png')

# Define your custom scoring function
def custom_scorer(y_true, y_pred):
    # Implement your custom scoring logic here
    # For example, let's use mean absolute error as a custom metric
