...
class TestEnv:
    ...
    @staticmethod
    def test_env(env, desc=None, render=False, n_iters=10, pi=None, user_input=False, goal_reward=1.0, convert_state_obs=lambda state: state):
        """
        Parameters
        ----------------------------
        ...
        success_steps = np.full([n_iters], np.nan)
        successes = np.full([n_iters], np.nan)
        success_step = None
        success = False
        for i in range(0, n_iters):
            ...
            success_steps[i] = success_step if success_step != None else n_step
            successes[i] = success
        env.close()
        return test_scores, success_steps, successes